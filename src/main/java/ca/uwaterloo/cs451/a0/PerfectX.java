...
/**
 * PerfectX exercise from word count demo.
 */
public class PerfectX extends Configured implements Tool {
  ...
  public static final class MyMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    public void map(LongWritable key, Text value, Context context)
      ...
      // Flag for finding the word "perfect" and set the default to false
      boolean PerfectFind=false;	
      for (String word : Tokenizer.tokenize(value.toString())) {
        // the word "perfect" is found as previous word
        if (PerfectFind){
	        WORD.set(word);
        ...
        // the word "perfect" find and set the flag to true
  ...
  public static final class MyReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
    ...
    public void reduce(Text key, Iterable<IntWritable> values, Context context)
      ...
      // only output the word with counts greater than 1
      if (sum > 1) {
        SUM.set(sum);
  ...
  public int run(String[] argv) throws Exception {
    ...
    // Remove combiner since the condition in the Reducer
