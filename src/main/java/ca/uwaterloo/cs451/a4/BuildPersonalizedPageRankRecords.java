...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static final String SOURCE_NUM_FIELD="source.num";

  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNodeArray> {
    private static final IntWritable nid = new IntWritable();
    private static final PageRankNodeArray node = new PageRankNodeArray();
    private static final List<Integer> sources = new ArrayList<Integer>();

    @Override
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNodeArray>.Context context) {
      int n = context.getConfiguration().getInt(NODE_CNT_FIELD, 0);
      String[] sourceNum = context.getConfiguration().get(SOURCE_NUM_FIELD).trim().split("\\s*,\\s*");
      ...
      if (sourceNum == null) {
        throw new RuntimeException(SOURCE_NUM_FIELD + " cannot be empty!");
      }
      // parse the input sourcenodes
      for(String str : sourceNum){
        sources.add(Integer.parseInt(str));
      }

      // initialize the node for non-source node
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      // check if the node has adjacency lists
      // and set up the adjacency list for the node
      if (arr.length == 1) {
        node.setNodeId(id);
      ...
      // set the node mass
      float[] pra=new float[sources.size()];
      for (int i=0; i<sources.size(); i++){
        if (sources.get(i) == id){
          // the mass log prob. would be log(1)
          pra[i]=(float) StrictMath.log(1);
        }
        else {
          // the mass log prob. would be log(0)
          pra[i]=(float) StrictMath.log(0);
        }
        node.setPageRankArray(new ArrayListOfFloatsWritable(pra));
  ...
  public int run(String[] args) throws Exception {
    ...
    String sourcesNum=cmdline.getOptionValue(SOURCES);

    LOG.info("Tool name: " + BuildPersonalizedPageRankRecords.class.getSimpleName());
    LOG.info(" - inputDir: " + inputPath);
    LOG.info(" - outputDir: " + outputPath);
    LOG.info(" - numNodes: " + n);
    LOG.info(" - sources: " + sourcesNum);

    Configuration conf = getConf();
    conf.setInt(NODE_CNT_FIELD, n);
    conf.set(SOURCE_NUM_FIELD, sourcesNum);
